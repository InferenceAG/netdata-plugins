...
SCRIPT_FILE="ETHERLINK_ETHERLINK_NETWORK.chart.py"
CONFIG_FILE="ETHERLINK_ETHERLINK_NETWORK.conf"
HEALTH_FILE="ETHERLINK_ETHERLINK_NETWORK.health.conf"
HEALTH_EVM_FILE="ETHERLINK_ETHERLINK_NETWORK.evm_health.conf"
HEALTH_ROLLUP_FILE="ETHERLINK_ETHERLINK_NETWORK.rollup_health.conf"
...
#	ETC_DIR=$(dirname $2)
	...
	echo "It will install ETHERLINK module to netdata daemon (or update it)"
...
if [ $# -eq 2 ] || [ $# -eq 0 ]; then
	if [ ! -d $MODULES_DIR ]; then
		...
		echo "Copying $HEALTH_EVM_FILE to $ETC_HEALTH_DIR" &&
		cp $(dirname $0)/$HEALTH_EVM_FILE $ETC_HEALTH_DIR/$HEALTH_EVM_FILE &&
		echo "Copying $HEALTH_ROLLUP_FILE to $ETC_HEALTH_DIR" &&
		cp $(dirname $0)/$HEALTH_ROLLUP_FILE $ETC_HEALTH_DIR/$HEALTH_ROLLUP_FILE &&		
		...
		chown netdata:netdata $ETC_HEALTH_DIR/$HEALTH_EVM_FILE &&
		chown netdata:netdata $ETC_HEALTH_DIR/$HEALTH_ROLLUP_FILE &&
		exit 0
	fi
fi
